...
const primaryColors = computed(() =>
    appConfig.ui.colors.filter(color => color !== 'primary').map(color => ({ value: color, text: color, hex: colors[color][colorMode.value === 'dark' ? 400 : 500] })))
...
onMounted(() => {
    const primary = window.localStorage.getItem('nuxt-ui-primary') || appConfig.ui.primary;
    appConfig.ui.primary = primary;

    const gray = window.localStorage.getItem('nuxt-ui-gray') || appConfig.ui.gray;
